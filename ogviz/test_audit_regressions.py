"""One test per defect found by reading the code, so none of them can come back.

Two read-throughs so far, 2026-07-27 and 2026-08-06, merged here because they are one kind of thing
and were drifting into two naming conventions — the second module was named after the date it was
written, which tells a later reader nothing and would have spawned a third name at the next audit.

What the defects have in common is why they are worth locking: every one passed the tests, the
linter and the type checker on the day it was found. None was a crash. Each was either silent — a
check that had quietly stopped checking — or plausible, a number that looked right.

Each test name says the failure rather than the fix, so a regression reads as the thing that came
back rather than as a repair that stopped working.
"""
