...
Fixed seeds: the gallery is regenerated by `just figures` (in the default `just` gate; CI lints
the examples and does not render them), and an image that changes on
...
def effort_ladders(seed: int = 12) -> dict[str, tuple[np.ndarray, np.ndarray]]:
    ...
    Not a sampled curve — four to six measured points per system, which is why the marks matter as
...
def scores_by_round(seed: int = 18) -> dict[str, tuple[list[float], list[tuple[float, float]]]]:
    ...
    `seed` is accepted for symmetry with the generators that draw, and ignored: the numbers are
    fixed, so passing a different seed changes nothing.
    """
    del seed
    return {
        "Acrobatics": (
...
def arm_comparison(seed: int = 19) -> dict[str, tuple[list[float], list[tuple[float, float]]]]:
    ...
    `seed` is accepted for symmetry with the generators that draw, and ignored.
