...
`uv run just figures` renders `examples/out/` from made-up data with fixed seeds, PNG and SVG. Source:
...
| ![](examples/out/03_stacked_brackets.png) | ![](examples/out/04_grouped_bars.png) |
| `bracket_stack` — stars anchored by their ink | `bar_panel` — signed bars, sign-aware labels |
| ![](examples/out/05_bars_with_reference.png) | ![](examples/out/06_horizontal.png) |
| asymmetric CI, a reference line, labels knock out what they cross | `orientation="horizontal"` |
| ![](examples/out/07_split_violins.png) | ![](examples/out/08_headline_bars.png) |
| `split_violins` — one quantity, two measurements | `rounded` / `highlight` / `reference_band` |
| ![](examples/out/09_coupling_panels.png) | ![](examples/out/10_violin_grid.png) |
| `coupling_panels` — pairs above, their estimates and stars on one scale below | a 2x2 grid of `group_violins` on one shared scale |
| ![](examples/out/11_effort_curves.png) | ![](examples/out/12_comparison_table.png) |
| `line_panel` — measured points, log money axis, `broken_zero` marks the cut | `table_panel` — highlighted column, shaded best value, `caption` |
