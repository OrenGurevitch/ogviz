...
| `table_panel` — a highlighted column or row, semantic cell tone, shaded best value | |
...
## Every figure, not just the ones you remembered

`save` audits what it writes. `guard()` makes that true of `fig.savefig` as well, so the checks run
whichever way a figure leaves — `plt.savefig`, a notebook, a helper that saves for you:

```python
import ogviz
ogviz.guard()                     # from here on, every savefig is audited
ogviz.guard(mode="warn")          # write it anyway and say what is wrong
ogviz.guard(mode="repair")        # fix what has one obvious fix first
```

`OGVIZ_GUARD=1` in the environment does the same with no code change, which is the form for CI and
for a paper build — `warn`, `repair` and `+advise` pick the mode. `guarded(...)` is the scoped
version, for a process that renders some figures under the house rules and some under its own.

It is NOT on for a bare `import ogviz`, deliberately: a library that changes matplotlib's behaviour
the moment it is imported would fail saves in code that never asked, break a notebook mid-session,
