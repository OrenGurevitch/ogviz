...
![A bar chart the gate refused, beside the complaints it raised](examples/out/13_the_gate.png)

<sub><b>6</b> a figure it refused, and why — every word on the right comes from <code>audit</code></sub>
...
- **`use_house_style()` once per process** — a second call resets the font family, so a project
  that pins its own font after the first call loses the pin; one that pins a font takes
  `use_house_ink()` alone, which is everything but the type. Pass `PAPER_WHITE` for a manuscript;
...
Plate 6, at the top of this page, is the gate at work.
...
│   │   ├── rounded_free_end(fraction: float, *, along: "Literal[x, y]")
│   │   └── value_labels(...) -> ...
│   ├── coupling
│   │   ├── Cloud(...) -> ...
│   │   ├── Estimate(...) -> ...
